(function () {
    ...
    function renderResults(entries, query) {
        ...
        if (!tokens.length) {
            renderEmptyState("输入关键词搜索标题、标签和正文。", "search-status-empty");
            return;
        }

        if (!entries.length) {
            renderEmptyState("没有找到匹配的文章。", "search-status-empty");
            return;
        }

        status.textContent = "找到 " + entries.length + " 篇与「" + query + "」相关的文章";
    ...
    fetch(indexUrl)
        ...
        .catch(function () {
            renderEmptyState("搜索索引加载失败，请刷新后重试。", "search-status-error");
        });
}());
